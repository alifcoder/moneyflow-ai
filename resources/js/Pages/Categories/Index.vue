...
const applyFilters = () => {
    router.get(route('categories.index'), cleanFilters(), {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const submit = () => {
    ...
    if (editingId.value) {
        form.put(route('categories.update', editingId.value), options);
...
const destroyCategory = (category) => {
    if (!category.can_delete || !window.confirm('Delete this category?')) {
        return;
    }
        ...
        <div class="py-6 sm:py-10">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    >
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span
                                ...
                                :class="category.owner === 'global' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'"
                            >
                                {{ category.owner === 'global' ? 'Global' : 'Own' }}
                            </span>
                        </div>
                ...
                <div class="hidden rounded-lg bg-white shadow-sm md:block md:overflow-x-auto">
                    <table class="w-full min-w-[42rem] divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="category in categories.data" :key="category.id">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700">{{ category.type }}</td>
                                <td class="px-4 py-3">
                                    <span
                                        class="rounded-full px-2 py-1 text-xs font-medium"
                                        :class="category.owner === 'global' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'"
                                ...
                                <td class="px-4 py-3 text-sm text-gray-700">
                                    {{ category.enabled ? 'Enabled' : 'Disabled' }}
                                    <span v-if="category.is_default"> · Default</span>
                                </td>
